...
while 1:
    ...
    try:
        ...
        if max_temp_pos[0] >=  lcd_w/2:
            ...
            x = max_temp_pos[0] + 4
        print((max/100.0))
        img = img.draw_rectangle(x, max_temp_pos[1], 80, 22, color=(0xff,112,0xff), fill=True)
        img = img.draw_string(x, max_temp_pos[1], "%.2f" %(max/100.0), color=(0xff,0xff,0xff), scale=2)
